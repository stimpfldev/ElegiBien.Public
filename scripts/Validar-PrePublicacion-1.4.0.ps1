...
$ZipPath = Join-Path $Artifacts "ElegiBien-1.4.0.zip"
$ShaPath = Join-Path $Artifacts "ElegiBien-1.4.0.zip.sha256"
...
if (-not (Test-Path $DbInstall)) {
    Fail "Database installer not found. Run scripts\Generar-Database-Install-1.4.0.ps1 first."
}

if ((Get-Item $DbInstall).Length -eq 0) {
    Fail "Database installer is empty."
}
...
Write-Host "Database installer: $DbInstall"
Write-Host ""
