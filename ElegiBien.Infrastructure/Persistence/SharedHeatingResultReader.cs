...
public class SharedHeatingResultReader : ISharedHeatingResultReader
{
    ...
    public async Task<SharedHeatingResultDto?> GetAsync(Guid analysisId, CancellationToken cancellationToken = default)
    {
        var calculation = await _dbContext.HeatingCalculationResults.AsNoTracking().SingleOrDefaultAsync(x => x.AnalysisId == analysisId, cancellationToken);
        if (calculation is null) return null;

        var alternatives = await GenericComparisonReader.LoadAsync(_dbContext, analysisId, CategoryCode.Heating, cancellationToken);
        var products = alternatives.Where(x => x.Score is not null).Select(x =>
        {
            ...
            return new HeatingProductScoreResultDto
            {
                ...
                CapacityStatus = GenericComparisonReader.GetStatus(score, HeatingCapacityStatus.Insufficient),
                IsEligible = score.IsEligible,
                AppliedMaximumScore = score.AppliedMaximumScore,
                Factors = score.Factors.Select(factor => new HeatingScoreFactorDto
                {
    ...
    private static string BuildRecommendation(IReadOnlyList<HeatingProductScoreResultDto> products)
    {
        if (products.Count == 0) return "Todavía no se compararon equipos para este análisis.";
